...
All ranges and bonuses are derived from the assumption that the player takes
proper time to hold and aim the firearm. Even for one handed guns (such as
pistols) it is assumed that the player uses both hands to hold and aim the
gun.

The recoil modifiers for one handed firearms (pistols) assume that they are
used with both hands. If they are shot from one hand, double the recoil base
value.

If the shooter fires without raising is gun for aiming (for example by firing
from the hip) the shooter loses the skill bonus to said firearm but may still
apply \emph{Perception} and \emph{Dexterity} to the roll. If he fires without
looking (blind fire) he loses any \emph{Perception} bonuses as well as the skill
bonuses. If the shooter moves during shooting the \emph{Dexterity}
bonus also does not apply.

Note that a shooter can aim, move and shoot at the same time. In such a case
only \emph{Dexterity} would be lost. If shooter moves and shoots without aiming
at his target (because the attention is diverted to the destination of the
movement), the shooter would lose, \emph{Dexterity}, \emph{Perception} and
\emph{Skill} bonuses. Any penalties listed below because of movement still
apply.
...
\emph{Misc. slots} are slots that can store equipment but are not as easily
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{| l | l | l | l | l | l |}
      ...
      Kevlar Vest     &   I & 40/40 & 200 & 2.0kg & \\ \hline
...
\label{sub:10-Non-lethal Grenades}
...
\subsection{Anti Vehicle Weapons}
\label{sub:10-Anti-Vehicle}

The table~\ref{tab:AntiVehicleWeapons} shows a list of man portable
anti vehicle weapons. These are intended to be used against small
armoured vehicles (such as tanks, armored trucks and cars, helicopters
or small landing craft). Many of these vary in shape, size and cost;
but most importantly in how they function.
...
weapon is fired. The lethal range of such a \emph{backblast} is usually given in
metres. Beyound that range anyone caught with the back blast takes \emph{2d6}
force and \emph{2d6} fire damage up until double the \emph{backblast} range.
Weapons with a \emph{backblast} are safe to fire from enclosed spaces as long
as a leeway of roughly 3 metres is given behind the shooter.

The table~\ref{tab:AntiVehicleWeaponsAmmo} lists the ammunition used by the
various launchers. If used against infantry they are considered to have a
\emph{kill radius} just like grenades. Beyound the \emph{kill radius} personnel
take the amount of damage given in the table. The penetration capabilities are
given in the highest possible armour rating this round can penetrate.

\begin{table}
  \caption{Anti Vehicle Weapons}
  \label{tab:AntiVehicleWeapons}
  \begin{center}
    \begin{tabular}{|l|ll|l|l|l|}
      \hline
      \textbf{Name} & \textbf{Cost} &  \textbf{Weight} &
      \textbf{Range} & \textbf{Recoil} & \textbf{Notes} \\ \hline

      RPG-32 & 8500 & 7.5 kg & 300m & 0 & Backblast 40m, Reloadable \\ \hline
      Pzf. 5 & 9000 & 6.0 kg & 300m & 0 & Backblast 40m, Reloadable \\ \hline
...
\begin{table}
  \caption{Anti Vehicle Weapons - Ammunition}
  \label{tab:AntiVehicleWeaponsAmmo}
  \begin{center}
    \begin{tabularx}{\textwidth}{|l|l|l|l|l|X|p{2.5cm}|}
      \hline
      \textbf{Name} & \textbf{Cost} &  \textbf{Weight} &
      \textbf{Radius} & \textbf{Used by} & \textbf{Damage} &
      \textbf{Penetration} \\ \hline


      PG-32VL  & 1000 & 2.0 kg &  5m & RPG-32 & 6d6 bludgeoning & TODO  \\ \hline
      PG-32VR  & 1500 & 3.2 kg &  5m & RPG-32 & 6d6 bludgeoning & TODO  \\ \hline
      OG-32V   &  900 & 1.5 kg & 30m & RPG-32 & 6d6 piercing and 6d6 bludgeoning & None. Anti-personnel. \\ \hline

      Pzf 5    & 1000 & 2.1 kg &  5m & Pzf. 5 & 6d6 bludgeoning & TODO  \\ \hline
      Pzf 5 IT & 1500 & 3.2 kg &  5m & Pzf. 5 & 6d6 bludgeoning & TODO  \\ \hline
    \end{tabularx}
  \end{center}
\end{table}
